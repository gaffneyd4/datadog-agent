...
type chooser struct {
	...
	// kubernetesReady determines if kubernetes is ready, or how long to wait
	kubernetesReady func() (bool, time.Duration)

	// dockerReady determines if docker is ready, or how long to wait
...
// ContainersOrPodsWait determines how the logs-agent should handle containers:
...
func ContainersOrPodsWait(ctx context.Context) LogWhat {
	singleton.start()
	select {
...
// ContainersOrPods is identical to ContainersOrPodsWait, except that it returns
// LogUnknown immediately in any situation where ContainersOrPods would wait.
...
func (ch *chooser) choose(wait bool) {
	...
	for {
		...
		case k && !c:
			ready, delay = ch.kubernetesReady()
			if ready {
				ch.choice <- LogPods
				return
			}
		...
		case k && c:
			...
			kready, kdelay := ch.kubernetesReady()
			switch {
			case dready && !kready:
				ch.choice <- LogContainers
				return
