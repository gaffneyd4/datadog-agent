...
import (
	...
	"github.com/DataDog/datadog-agent/pkg/logs/internal/util/containerorpods"
...
func (l *Launcher) Stop() {
	...
	if containerorpods.ContainersOrPods() == containerorpods.LogPods {
		l.stopped <- struct{}{}
	}
}
...
func (l *Launcher) run(sourceProvider launchers.SourceProvider, pipelineProvider pipeline.Provider, registry auditor.Registry) {
	...
	if containerorpods.ContainersOrPodsWait(l.ctx) != containerorpods.LogPods {
		return
	}
	...
	// kubeutil should be available now, as ContainersOrPodsWait waits until that
	// is the case.
	var err error
